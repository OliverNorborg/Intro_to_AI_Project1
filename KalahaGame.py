...
"""Building the tree for the minmax evaluation"""        
...
class Tree:
    ...
    def build_tree(self, node, recursion=0):
        ...
        for i in range(6):
            board = copy.deepcopy(Node().getdata()) #These nodes will all be siblings
            print("board", board)
            if board.choose_bowl(i):
                if not board.emptyside():
                    node.addchild(action=i, child=Leaf(board))
                else:
                    node.addchild(action=i, child=Node(board))
                    print("action",i)
                    print("child", Node(board))
        for child in node.getchild().getvalues():
