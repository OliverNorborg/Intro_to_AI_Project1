...
class Game:
    def __init__(self):
        self.balls = 6
        self.state = {0: [0,0,0,0,2,9,0], 1: [0,0,0,3,11,0,0]}
        self.playersturn = 0
...
for bowl, balls in enumerate(state[playersturn][0:6]):
    opponentsturn = 1 if playersturn == 0 else 0
    print("FIRST FOR LOOP: bowls", bowl+1,"balls", balls)
    pieces_needed = (6 - bowl) - balls
    #print("Pieces needed:", pieces_needed)
    steel = state[opponentsturn][abs(5-bowl)]
    print("steel", steel)
    if balls == 0 and steel > 0:
        for bowl2, balls2 in enumerate(state[playersturn][0:bowl]):
            print("SECOND FOR LOOP: bowls2", bowl2+1,"balls2", balls2)
            #print("bowls2")
            if (bowl - bowl2) == balls:
                print()
                print("In the if statement",steel)
                print()
